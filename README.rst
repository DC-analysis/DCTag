...
where ``X.Y.Z`` is the version of DCTag you are interested in. E.g. to install DCTag 0.4.2 via SSH
(works if you have two-factor authentication enabled), run::

    pip install dctag@git+ssh://git@gitlab.gwdg.de/blood_data_analysis/dctag.git@0.4.2

Windows users please note that this might only work with git bash.

To **upgrade** to a new version, use the ``--upgrade`` argument::

    pip install --upgrade dctag@git+ssh://git@gitlab.gwdg.de/blood_data_analysis/dctag.git@0.4.3
